...
par(mfrow = c(2,2))
## plot 1/4
plot(strptime(feb_time,"%Y-%m-%d %H:%M:%S"), active_pwr/500, type="n", yaxt="n", ylab="Global Active Power", xlab="")
lines(strptime(feb_time,"%Y-%m-%d %H:%M:%S"), active_pwr/500)
axis(2, at=c(0,2,4,6))

## plot 2/4
voltage <- as.numeric(feb_pwr$Voltage)
plot(strptime(feb_time,"%Y-%m-%d %H:%M:%S"), voltage/600, type="n", yaxt="n", ylab="Voltage", xlab="")
lines(strptime(feb_time,"%Y-%m-%d %H:%M:%S"), voltage/600)
